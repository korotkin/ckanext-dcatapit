...
class DCATAPITCommands(CkanCommand):
    ...
    def command(self):
        ...
        if cmd == 'load':
            ...
            log.error('ERROR: Command "{0}" not recognized', cmd)
            log.debug(self.usage)
            return
    ...
    def load(self):
        ...
        if not vocab_name:
            log.error('ERROR: No vocabulary name provided and is required')
            log.debug(self.usage)
            return

        if vocab_name not in self._controlled_vocabularies_allowed:
            log.error(
                'ERROR: Incorrect vocabulary name, only one of these values are allowed: {0}',
